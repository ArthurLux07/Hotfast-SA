...
function exibirProdutos() {
    ...
    for (let i in produtos) {
        ...
        adicionarAoCarrinhoBtn.onclick = (i) => adicionarAoCarrinho(i);
...
/*function adicionarAoCarrinho(i) {
   ...
   const textoCarrinhoVazio = document.getElementById('textoCarrinhoVazio');
    ...
    const produtoQueOCaraClicou = listaProdutos[i]; //rever essa lógica
    ...
    for (let i in produtosCarrinho) {
        ...
        botaoDeletarDoCarrinho.onclick = (index) => deletarDoCarrinho(index);
...
function deletarDoCarrinho(index) {
    let produtosCarrinhoRemover = JSON.parse(localStorage.getItem('produtos')) || [];
    produtosCarrinhoRemover.splice(index, 1);
    localStorage.setItem('produtos', JSON.stringify(produtosCarrinhoRemover));
    adicionarAoCarrinho();
}
